...
public class MyDiskLruCache {
    public static final String TAG = "MyDiskLruCache";
    ...
    private MyDiskLruCache(File cacheDir, long maxByteSize) {
        
        mCacheDir = cacheDir;
        maxCacheByteSize = maxByteSize;
    }
    
    public static MyDiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        if(!cacheDir.exists()){
            cacheDir.mkdir();
        }
        if(cacheDir.isDirectory() && cacheDir.canWrite()
                && MyUtils.getUsableSpace(cacheDir) > maxByteSize){
            
            return new MyDiskLruCache(cacheDir, maxByteSize);
        }
        return null;
    }
    ...
    private boolean writeBitmapToFile(Bitmap bitmap, String file)
        ...
        try {
            out = new BufferedOutputStream(new FileOutputStream(file), MyUtils.IO_BUFFER_SIZE);
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath =
                Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED ||
                        !MyUtils.isExternalStorageRemovable() ?
                        MyUtils.getExternalCacheDir(context).getPath() :
