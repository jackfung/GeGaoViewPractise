...
class MyBrowserBookmarksAdapter extends BaseAdapter {
    ...
    private int                     mCount;
    private MyBrowserBookmarksPage    mBookmarksPage;
    ...
    /**
     *  Create a new MyBrowserBookmarksAdapter.
     *  @param b        MyBrowserBookmarksPage that instantiated this.
     *                  Necessary so it will adjust its focus
     *                  appropriately after a search.
     */
    public MyBrowserBookmarksAdapter(MyBrowserBookmarksPage b, String curPage,
            String curTitle, Bitmap curThumbnail, boolean createShortcut,
            boolean mostVisited, boolean isHome) {
        if (isHome)
            mNeedsOffset= false;
        else
    ...
    public void deleteRow(int position) {
        ...
        MyBookmarks.removeFromBookmarks(null, mContentResolver, url, title);
        refreshList();
    }
    ...
    /* package */ void populateBookmarkItem(MyBookmarkItem b, int position) {
        mCursor.moveToPosition(position - mExtraOffset);
        String url = mCursor.getString(Browser.HISTORY_PROJECTION_URL_INDEX);
        b.setUrl(url);
        ...
        Bitmap bitmap = null;
        if (data == null) {
            bitmap = MyCombinedBookmarkHistoryActivity.getIconListenerSet()
                    .getFavicon(url);
        } else {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position < 0 || position > mCount) {
            throw new AssertionError(
                    "MyBrowserBookmarksAdapter tried to get a view out of range");
        }
        if (mViewMode == BookmarkViewMode.GRID) {
            if (convertView == null || convertView instanceof MyAddNewBookmark
                    || convertView instanceof MyBookmarkItem) {
            ...
            if (0 == position && mNeedsOffset) {
                ...
                if (mCurrentThumbnail != null) {
                    thumb.setImageBitmap(Utils.getRoundedCornerBitmap(mCurrentThumbnail, 5,true));
                } else {
            ...
            if (thumbnail == null) {
                thumb.setImageResource(R.drawable.bookmark_default);
            } else {
                thumb.setImageBitmap(Utils.getRoundedCornerBitmap(thumbnail, 5,false));
            }
        ...
        if (position == 0 && mNeedsOffset) {
            MyAddNewBookmark b;
            if (convertView instanceof MyAddNewBookmark) {
                b = (MyAddNewBookmark) convertView;
            } else {
                b = new MyAddNewBookmark(mBookmarksPage);
            }
            b.setUrl(mCurrentPage);
            return b;
        }
        if (mMostVisited) {
            if (convertView == null || !(convertView instanceof MyHistoryItem)) {
                convertView = new MyHistoryItem(mBookmarksPage);
            }
        } else {
            if (convertView == null || !(convertView instanceof MyBookmarkItem)) {
                convertView = new MyBookmarkItem(mBookmarksPage);
            }
        }
        bind((MyBookmarkItem) convertView, position);
        if (mMostVisited) {
            ((MyHistoryItem) convertView).setIsBookmark(
    ...
    public byte[] getByteArrayOfScreenshot(int position) {
        return getByteArray(Browser.HISTORY_PROJECTION_THUMBNAIL_INDEX, position);
    }
    ...
    public byte[] getByteArrayOfTouchIcon(int position) {
        return getByteArray(Browser.HISTORY_PROJECTION_TOUCH_ICON_INDEX, position);
    }
    
    private byte[] getByteArray(int cursorIndex, int position) {
        if (position < mExtraOffset || position > mCount) {
            return null;
        }
        mCursor.moveToPosition(position - mExtraOffset);
    ...
    private void bind(MyBookmarkItem b, int position) {
        mCursor.moveToPosition(position- mExtraOffset);
        ...
        if (data != null) {
            ...
            b.setFavicon(MyCombinedBookmarkHistoryActivity.getIconListenerSet()
                    .getFavicon(url));
        }
    }
